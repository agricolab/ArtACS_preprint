...
Besides the discussed real-valued causal comb filters, several other approaches for filtering the~\gls{tacs}-artifact are possible. For example, one could estimate the weight at each period delay by calculating the autocorrelation~\ref{sec:automaticKernel}, remove periodic principial components~\ref{sec:adaptivePCA}, or use complex-valued kernels~\ref{sec:causalDFT}.

Consider that these approaches are either computationally more complex~\ref{sec:automaticKernel}, work primarily offline~\ref{sec:adaptivePCA} or assume sinusoidality of the artifact~\ref{sec:causalDFT},the causal methods presented here are conceptually simpler.

\subsubsection{Automatic Weight Estimation}\label{sec:automaticKernel}

By cutting a trial into epochs of period length, we can estimate the average autocorrelation for each phase across different (period length) lags. Because this measure is a direct indicator of the reliability of earlier measurements for estimating the current artifact amplitude, we can use it to derive empirical kernel weights.

\subsubsection{Causal Discrete Fourier Transformation}\label{sec:causalDFT}

To get hold of the artifacts no-stationarity, we might also approach by iteratively estimating for each time point the artifact power at the frequency of~\gls{tacs}, e.g.\ by using a discrete fourier transformation with a window over a certain number of period periods, and use that for scaling the artifact template to be removed.
The estimation-removal-process can be understood as convolution of the recording with a complex kernel at the artifacts frequency and subsequent subtraction.
While it has the advantage that it supports natively frequencies which are not integer divisibles of the sampling frequency, we need to assume sinusoidality of the artifact. Additionally, to remove the artifact at harmonic frequencies, the computation has to be performed several times.
Whether this approach is sufficient, will depende on whether the artifact can be sufficiently catched by these few frequencies.

\subsubsection{Adaptive Removal of Principal Components}\label{sec:adaptivePCA}

Instead of focussing on the artifacts frequency in sinusoidal terms, we might focus on its periodicity and estimate the modulation of the artifacts periodic components across time.
By cutting a trial into epochs of period length, we can perform a prinical component analysis.
If the trial duration is significantly larger than the duration of the physiological signal, we can safely assume that the physiological signal will only span a small subspace with small eigenvalues.
Removing the largest principal components is then likely to reduce the artifact, but not the signal. Besides an arbitrary threshold (e.g.\ only the first, or three), we can also iteratively remove components until the power in the artifacts frequency is sufficiently reduced to par or below the power of nearby frequencies.

\section{Evaluation}

We implemented several of the discussed functions for kernel creation and artifact removal in Matlab 2016b.\footnote{Code can be accessed freely, in open-source and online at \url{https://github.com/agricolab/ARtACS} and has been licensed under a X11-license.}
We selected various kernels and evaluated their frequency response characteristics~(see~\ref{sec:EvaluationFreq}) as well as their performance on simulated~(see~\ref{sec:EvaluationSimulated}) and real data~(see~\ref{sec:EvaluationData}).
